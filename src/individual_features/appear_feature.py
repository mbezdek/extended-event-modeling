...
def gen_appear_features(args, run, tag):
    try:
        ...
        with open(f'output/appear_complete_{tag}.txt', 'a') as f:
            f.write(run + '\n')
        return input_csv_tracking, output_csv_appear
    except Exception as e:
        with open(f'output/appear_error_{tag}.txt', 'a') as f:
            f.write(run + '\n')
            f.write(repr(e) + '\n')
...
if __name__ == '__main__':
    ...
    if os.path.exists(f'output/appear_complete_{args.feature_tag}.txt'):
        os.remove(f'output/appear_complete_{args.feature_tag}.txt')
    if os.path.exists(f'output/appear_error_{args.feature_tag}.txt'):
