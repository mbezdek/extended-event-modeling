...
complete_txt = f"preprocessed_complete_{args.feature_tag}.txt"
if os.path.exists(complete_txt):
    print(f"{complete_txt} existed, deleting..")
    os.remove(complete_txt)
error_txt = f"preprocessed_error_{args.feature_tag}.txt"
if os.path.exists(error_txt):
    print(f"{error_txt} existed, deleting..")
    os.remove(error_txt)
filtered_txt = f"filtered_skel_{args.feature_tag}_{args.ratio_samples}_{args.ratio_features}.txt"
