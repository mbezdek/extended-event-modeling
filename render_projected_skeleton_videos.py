...
#LOCAL:
skel_dir="/Users/bezdek/Documents/projected_skeletons/"
...
videofolder='/Users/bezdek/Box/DCL_ARCHIVE/Project-SEM-Corpus/trimmed_videos/'
...
def bodystats(df):
    ...
    while i < len(bodies):
        ...
        tempd={'id':bodies[i],'min':np.min(temp[0]),'max':np.max(temp[0])}
...
#Resampling function:
def skelresample(testdf,rate='40ms',small=True):
    ...
    for col in tracklist2D[::2]:
        #outdf[col]=outdf[col].dropna().astype(np.int64)
        if small:
            outdf[[col]]=np.where(outdf[[col]] > 960,960,outdf[[col]])
        else:
            outdf[[col]]=np.where(outdf[[col]] > 1920,1920,outdf[[col]])
    for col in tracklist2D[1::2]:
        #outdf[col]=outdf[col].dropna().astype(np.int64)
        if small:
            outdf[[col]]=np.where(outdf[[col]] > 540,540,outdf[[col]])      
        else:
            outdf[[col]]=np.where(outdf[[col]] > 1080,1080,outdf[[col]])
...
#  
skiplist=[]
i=0 #body index for choosing skeleton color
for skelfile in skelfiles:
    cam=skelfile[-11:-9]
    print(' ')
    print('***')
    print(os.path.splitext(os.path.basename(skelfile))[0][0:-8],cam)
    ...
    #else:
    videofile=videofolder+os.path.splitext(os.path.basename(skelfile))[0][0:-8]+'_'+cam+'_trim.mp4'
    vidout=outdir+os.path.splitext(os.path.basename(skelfile))[0][0:-8]+'_'+cam+'_skel.avi'
    if (os.path.isfile(vidout)):
        ...
        #if os.path.splitext(os.path.basename(skelfile))[0][0:-8] in ['1.3.3','1.3.4','1.3.9','1.3.1','2.3.10','6.2.1','6.2.7','6.2.5']:
        #    skeldf=skelresample(skeldf,rate='33366667ns')
        #else:
        #    skeldf=skelresample(skeldf,rate='40ms')
        # C1 and C2 videos are 59.940060 frames per second:
        #skeldf=skelresample(skeldf,rate='16683333ns')
        # Small C1 and C2 videos are 30 frames per second:
        if small_vid:
            skeldf=skelresample(skeldf,rate='33333333ns',small=True)
        else:
            skeldf=skelresample(skeldf,rate='16683333ns',small=False)
        #correct for asynchrony:
        skeldf['sync_time']=skeldf['sync_time']+0.19
