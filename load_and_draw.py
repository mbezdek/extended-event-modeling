...
def load_and_draw(input_video_path, input_segmentation, output_dir, output_video_name, region_length):
    ...
    seg['segment_processed'] = seg['segment_processed'].apply(lambda s: s.split('\n'))
    seg['segment_processed'] = seg['segment_processed'].apply(lambda l: [float(e.split(' ')[1]) for e in l
                                                                         if 'BreakPoint' in e])
    ...
    # calculate segmentation points according to fps, consider only one annotation
